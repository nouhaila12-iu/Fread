...
import com.fread.freadbackend.model.*;
import com.fread.freadbackend.repository.*;

import java.util.List;

@RestController
@CrossOrigin
public class FreadRestController
{
    @Autowired
    ...
    public String deleteCourse(@PathVariable Integer id)
    {
        if(courseRepository.existsCourseByCourseid(id)) {
        	Course crs = courseRepository.findCourseByCourseid(id);	
        	if (crs.getEnrolls()!=0) {
        		return "there's students enrolled in this course ,it can't be deleted ";
    ...
    public Course updateCourseDetails(@RequestBody Course course)
    {
        ...
        edit_course.setDateovr(course.getDateovr());
        edit_course.setDatefrm(course.getDatefrm());
        edit_course.setNbh(course.getNbh());
        try {
    ...
    public String updateEnrolls(@PathVariable Integer id)
    {
        ...
        try {
            int x = edit_course.getEnrolls() + 1;
            edit_course.setEnrolls(x);
            courseRepository.save(edit_course);
    ...
    @PostMapping(value = "/user/update/enrolled-course")
    public User updateEnrolledCourse(@RequestBody User user)
    {
        User edit_user = userRepository.findUserByUserid(user.getUserid());
        edit_user.setEnrolled_courses(user.getEnrolled_courses());
        userRepository.save(edit_user);
